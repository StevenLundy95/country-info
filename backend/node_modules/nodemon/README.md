...
```bash
nodemon ./backend.js localhost 8080
```
...
```bash
nodemon --watch app --watch libs app/backend.js
```
...
```bash
nodemon --delay 10 backend.js
```

For more precision, milliseconds can be specified.  Either as a float:

```bash
nodemon --delay 2.5 backend.js
```

Or using the time specifier (ms):

```bash
nodemon --delay 2500ms backend.js
```
...
```bash
nodemon --signal SIGHUP backend.js
```
